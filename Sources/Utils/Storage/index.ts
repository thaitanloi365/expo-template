...
/**
 * Burn it all to the ground except key.
 */
export async function clearExcept(key: string) {
  try {
    const exceptValue = await load(key);
    await AsyncStorage.clear();
    await AsyncStorage.setItem(key, JSON.stringify(exceptValue));
    return true;
  } catch {
    return false;
  }
